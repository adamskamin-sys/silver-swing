...
2026-07-14 18:00  LOCAL  commit  57c3065 — diag_fix_slr_primary safety: added missing sys import + two-signal alive check (live_order_id set OR fresh heartbeat < 60s → REFUSE unless --force). Adam's earlier run bypassed the guard because sys wasn't imported and heartbeat threshold was too tight.
2026-07-14 18:05  ADAM   action  Re-ran diag_fix_slr_primary --confirm; NEW live_order_id in state = 'f7f11c9a-2a6f-4b77-9638-63e4a4737349'. Guard didn't fire because sys was missing at that point. Third re-arm since fix attempts.
2026-07-14 18:10  LOCAL  commit  d152195 — Halt banner: inline Resume button per halted product. Adam's UX blocker (no sleeve = no row = no Resume button) closed by moving the button to the banner itself with warning tooltip.
2026-07-14 18:15  LOCAL  commit  0c18d6e — reconciliation_monitor: include core_qty + per-sleeve rows in expected_position. Prevents false-alarms on products with core holdings (previously would fire position_mismatch for every product where Coinbase qty = core qty because expected only counted primary swing_qty).
2026-07-14 18:20  CLOUD  gate    Sent review gate for reentry_reeval wiring: Preconditions (flag off = byte-for-byte legacy; paper first), Tier 1 (cancel-before-place / dedup lock / anti-thrash), Tier 2 (state persist memory+Redis / expire clean / unified level logic), Tier 3 (inputs single source / near_expiry / trade-log events).
2026-07-14 18:35  LOCAL  branch  feat/reentry-reeval-wiring · 145753a — arm_level.py (shared pullback_buy_px helper) + swing_leg.py _maybe_reeval_pending_arm + _reeval_cancel_replace + _reeval_expire. 17 new tests hitting every Tier 1+2 checkbox. Byte-for-byte legacy when flag off. NOT MERGED — awaits auditor + Adam sign-off per gate.
2026-07-14 18:45  LOCAL  commit  82f614f — reconciliation_monitor.check_state_config_drift closes the SLR-ghost class (auditor's specific ADD from cache-coherence agenda). Wired into live_runner; would have caught today's SLR ghost within 5 min.
2026-07-14 18:50  LOCAL  commit  d23ea47 — Cockpit tile for reconciliation findings. Visual surface for the read-only defense: red 🚨 if critical, amber ⚠ if warn only, ✓ if clean. Tooltip lists the kinds. 15-min window scoped to active tenant.
